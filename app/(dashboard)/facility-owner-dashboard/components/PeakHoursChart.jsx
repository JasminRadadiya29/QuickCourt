...
const PeakHoursChart = () => {
  ...
  useEffect(() => {
    const fetchPeakHoursData = async () => {
      try {
        setLoading(true);
        const response = await axios.get('/api/owner/analytics/peak-hours');
  ...
  const peakHour = peakHoursData && peakHoursData.length > 0 
    ? peakHoursData.reduce((max, current) => 
        current?.bookings > max?.bookings ? current : max, peakHoursData[0])
    : { hour: 'N/A', bookings: 0 };
